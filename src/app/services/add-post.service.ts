...
import {PostPayload} from "../models/ghof/post-payload";
...
export class AddPostService {
  ...
  baseUrl = 'http://localhost:8181/post';
  ...
  getCommentList(): Observable<any[]> {
    return this.httpClient.get<any>('http://localhost:8181/api/comment/getall');
  }
  ...
  addPostcom(comment: any,iduser:any,idcs:any): Observable<any> {
    return this.httpClient.post(`http://localhost:8181/api/comment/addPost/${iduser}/${idcs}`, comment);
  }
  saveComment(comment: any, postId: number): Observable<any> {
    return this.httpClient.post(`http://localhost:8181/api/comment/add/${postId}`, comment);
  }
  ...
  getAllPosts():Observable<Array<PostPayload>>{
    return this.httpClient.get<Array<PostPayload>>("http://localhost:8181/post/getAll");
  }
  ...
  getPost(permaLink:Number):Observable<PostPayload>{
    return this.httpClient.get<PostPayload>('http://localhost:8181/post/get/'+ permaLink);
  }
  post_options = {
    method: 'POST',
    headers: {
